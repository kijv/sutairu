...
describe('Numeric Values', () => {
  test('Authors can use numeric values to assign px values', () => {
    ...
    expect(toString()).toBe(
      '--sxs{--sxs:1 cSHHDh fFIrKk}@layer global{body{margin:0}body{margin:10px}}',
    );
  });
  ...
  test('Authors can use numeric values to assign numeric values', () => {
    ...
    expect(toString()).toBe(
      '--sxs{--sxs:1 bpctHq}@layer global{body{line-height:0;width:0}}',
    );
    ...
    expect(toString()).toBe(
      '--sxs{--sxs:1 bpctHq cudWGu}@layer global{body{line-height:0;width:0}body{line-height:10;width:10px}}',
    );
  });
  ...
  test('Authors can use unit-less properties as known to React', () => {
    for (let i = 0; i <= 33; i += 11) {
      ...
      const cssText = getCssText().replace(
        /^.+@layer\s(themed|global|styled|onevar|reasonevar|allvar|inline)\{|\}$/g,
        '',
      );
