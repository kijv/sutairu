...
describe('Prefix', () => {
  ...
  test('Authors can define a prefix applied to themes', () => {
    ...
    expect(toString()).toBe(
      `--sxs{--sxs:0 fusion-t-iknykm}@layer themed{.${prefix}-t-${hash}{--fusion-colors-red:tomato}}`,
    );
  });
  ...
  test('Authors can define a prefix not applied to named themes', () => {
    ...
    expect(toString()).toBe(
      `--sxs{--sxs:0 my-theme-name}@layer themed{.${themeName}{--fusion-colors-red:tomato}}`,
    );
  });
  ...
  test('Authors can define a prefix applied to components', () => {
    ...
    expect(toString()).toBe(
      '--sxs{--sxs:2 fusion-c-gmqXFB}@layer styled{.fusion-c-gmqXFB{color:red}}',
    );
  });
});
